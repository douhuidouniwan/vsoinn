...
class Soinn(BaseEstimator, ClusterMixin):
    ...
    def _reset_state(self):
        ...
        self.nodeMark = np.array([], dtype=np.int32)
        self.winning_times = []
    ...
    def fit(self, X):
        ...
        numberMark = 0
        for x in X:
            self.input_signal(x,numberMark)
    ...
    def input_signal(self, signal: np.ndarray, numberMark, signal1=np.array([], dtype=np.float64)):
        """
        ...
        if self.nodes.shape[0] < self.init_node_num:
            # todo
            # signal1[-1,0:signal.size] = signal
            # signal1[-1,signal.size] = numberMark
            signal1=signal.copy()
            signal1.resize(1, signal.size+1, refcheck=False)
            signal1[-1,signal.size] = numberMark
            self.__add_node(signal,numberMark)
            return
    ...
    def __add_node(self, signal: np.ndarray,numberMark):
        n = self.nodes.shape[0]
        self.nodes.resize(n + 1, self.dim, refcheck=False)
        self.nodes[-1, :] = signal
        # todo
        self.nodeMark.resize(n + 1, 1, refcheck=False)
        self.nodeMark[-1, :] = numberMark
    ...
    def __delete_nodes(self, indexes):
        ...
        # todo
        self.nodeMark = np.delete(self.nodeMark, indexes, 0)
